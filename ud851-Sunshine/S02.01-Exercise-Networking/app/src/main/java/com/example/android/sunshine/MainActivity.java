...
//        }
        ...
        loadWeatherData(NetworkUtils.buildUrl("Shanghai"));
    }

    // TODO (8) Create a method that will get the user's preferred location and execute your new AsyncTask and call it loadWeatherData
    private void loadWeatherData(URL url) {
        new fetchWeatherData().execute(url);
    }


    // TODO (5) Create a class that extends AsyncTask to perform network requests
    public class fetchWeatherData extends AsyncTask<URL, Void, String> {

        @Override
        protected String doInBackground(URL... urls) {
            URL url = urls[0];
            String fetchWeatherResults = null;
            try {
                fetchWeatherResults = NetworkUtils.getResponseFromHttpUrl(url);
            } catch (IOException e) {
                e.printStackTrace();
            }
